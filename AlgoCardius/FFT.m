...
EpochTime = 3; %10Sec
SamplingFrequency = 250 ; %250Hz
EpochSampleLength = EpochTime*SamplingFrequency; %750 Samples in a 3Sec epcohs
SamplingTime = 1/SamplingFrequency; %4mSec
TimeForEpoch = 0:SamplingTime:EpochTime-SamplingTime;
New_DataDCT = zeros(1,750);
ZCTRes = zeros(1,2097);
Res = zeros(1,2097);
...
for I = 01:1:10
    for pos = 1:2097
        TestData = ATM426(1,1+(250*pos):EpochSampleLength+(250*pos));
        Data = TestData;
        DataDCT=dct(Data);
        na=std(DataDCT);
        for k=1:length(DataDCT)
        ...
        sn=New_Data;
        wn=ones(1,length(sn));
        wn=(-1).^(0:length(wn)-1)*I;    
        zn=sn+wn;
        somme=0;
        for i=2:length(zn)
            somme=somme+abs(sign(zn(i))-sign(zn(i-1)));
        end
        ZCR=(1/(2*length(zn)))*somme;
    ...
    iter = 2097;
    for i = 1:iter
       if  ((ComArray426(i)-78)/5) == Res(i)
           perc = perc+1;
       end
    end
    perc = perc / iter
    percRes(iterNumb) = perc;
    iterNumb = iterNumb +1
